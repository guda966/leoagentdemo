...
serve(async (req) => {
  ...
  try {
    const { title, skills, experience, location } = await req.json();
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are an expert HR recruiter. Generate professional, engaging job descriptions." },
        ...
        tool_choice: { type: "function", function: { name: "generate_jd" } },
      }),
    });
    ...
    console.error("job description error:", e);
